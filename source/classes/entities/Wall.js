export default class Wall {
    ...
    draw(ctx) {
        if (this.exploding) {
            ...
            if (this.emerging) {
                const shake = this.shakeIntensity * (1 - this.emergingTimer / this.emergingDuration);
            ...
            ctx.lineWidth = 1;
            ctx.strokeRect(this.x - 1, drawY - 1, this.width + 2, this.currentHeight + 2);
        ...
        if (particlesArray) {
            const ExplosionParticle = class {
                constructor(x, y) {
                    ...
                    this.color = ['#8800ff', '#aa00ff', '#ff00ff'][Math.floor(Math.random() * 3)];
                    this.life = 30;
                    this.maxLife = 30;
                }
            ...
            for (let i = 0; i < 30; i++) {
                particlesArray.push(new ExplosionParticle(
                    this.x + this.width / 2,
